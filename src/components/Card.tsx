...
export const Card = ({ title, description, techs = [], image }: CardProps) => {
  ...
  return (
    <motion.div className="flex flex-col w-full h-full bg-[#04071D] p-4"
    variants={containerVariants}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.5 }}>
      <motion.div className="flex relative justify-center items-center overflow-hidden w-full h-[100%] bg-[#13162D] rounded-2xl"
      ...
      <motion.div className="flex flex-col w-full items-center h-[60%]"
        variants={itemVariants}>
      ...
      <motion.div className="flex w-full justify-between items-center px-2 md:px-4 pb-4"
        variants={containerVariants}
      >
        {techs.map((tech) => (
          <motion.div
